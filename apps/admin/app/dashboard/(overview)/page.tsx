...
export default async function DashboardPage(props: {
  ...
  let selectedBranchId = searchParams.branch_id as string;
  if (!selectedBranchId) {
    if (authUser?.branch_id) {
      selectedBranchId = authUser.branch_id;
    } else {
      // Fetch the main branch or first branch dynamically!
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">{isAdmin && <DailyReportToggle />}</div>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {operational.cards.map((card) => {
        ...
        const hasIssues =
          card.orderCount > 0 && (card.icon === 'alert-triangle' || card.icon === 'clock-alert');
        ...
        return (
          <Link key={card.label} href={card.filterUrl}>
            <Card
              className={`border shadow-sm hover:shadow-md transition-all cursor-pointer ${hasWarning ? `${colors.bg} ${colors.border}` : 'bg-white border-slate-200 hover:border-slate-300'}`}
            >
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  ...
                  {isProgressCard ? (
                    card.totalCount! > 0 ? (
                      <span
                        className={`text-3xl font-black ${hasWarning ? colors.text : 'text-slate-900'} tabular-nums`}
                      >
                    ...
                    <>
                      {card.orderCount > 0 && (
                        <div className="flex flex-col items-end">
                          <span
                            className={`text-3xl font-black ${hasIssues ? colors.text : 'text-slate-900'} tabular-nums`}
                          >
                            {card.orderCount}
                          </span>
                ...
                <div className="mt-3">
                  <h3
                    className={`text-sm font-semibold ${hasWarning ? colors.text : 'text-slate-900'}`}
                  >
                    {card.label}
                  </h3>
                  <p
                    className={`text-xs mt-0.5 ${hasWarning ? `${colors.text} font-bold` : 'text-slate-500'}`}
                  >
                    {isProgressCard
                      ? card.totalCount === 0
                        ? 'All clear'
  ...
  return (
    <div className="space-y-6">
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] font-bold flex items-center justify-end gap-1 mt-0.5 uppercase tracking-wider">
                  ...
                  <span
                    className={
                      metrics.salesPacing.isPositive ? 'text-emerald-600' : 'text-rose-600'
                    }
                  >
          ...
          <Card className="border-0 shadow-sm bg-white overflow-hidden hover:ring-2 hover:ring-emerald-500/20 transition-all cursor-pointer">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] font-bold flex items-center justify-end gap-1 mt-0.5 uppercase tracking-wider">
                  ...
                  <span
                    className={
                      metrics.revenuePacing.isPositive ? 'text-emerald-600' : 'text-rose-600'
                    }
                  >
            ...
            <CardContent className="pt-2">
              <p className="text-xs text-slate-500 mb-3">
                Total money actually received (Advance + Final) during this period across all
                payment modes.
              </p>
              <div className="flex items-center gap-4 mt-1 bg-slate-50 p-2 rounded-lg overflow-x-auto scrollbar-hide">
                <div className="flex flex-col min-w-fit">
                  <span className="text-[9px] text-slate-400 uppercase font-bold leading-none mb-1">
                    Cash
                  </span>
                  <span className="text-xs font-black text-slate-900">
                    {formatCurrency(metrics.total_cash)}
                  </span>
                </div>
                ...
                <div className="flex flex-col min-w-fit">
                  ...
                  <span className="text-xs font-black text-slate-900">
                    {formatCurrency(metrics.total_upi)}
                  </span>
                </div>
                <div className="w-px h-6 bg-slate-200 shrink-0" />
                <div className="flex flex-col min-w-fit">
                  ...
                  <span className="text-xs font-black text-slate-900">
                    {formatCurrency(metrics.total_gpay)}
                  </span>
                </div>
                <div className="w-px h-6 bg-slate-200 shrink-0" />
                <div className="flex flex-col min-w-fit">
                  ...
                  <span className="text-xs font-black text-slate-900">
                    {formatCurrency(metrics.total_bank_transfer)}
                  </span>
                </div>
              </div>
            </CardContent>
        ...
        <CardContent className="h-[200px] flex items-end gap-1.5 pt-6 px-6">
          {metrics.dailyRevenue.map((day, i) => {
            ...
            return (
              <div
                key={i}
                className="flex-1 bg-slate-50 hover:bg-slate-100 rounded-t-md relative group transition-all duration-300"
                style={{ height: '100%' }}
              >
                <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="border-0 shadow-sm bg-white hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs mt-2 text-slate-500">
              Revenue from orders that are fully returned and closed.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm bg-white hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs mt-2 text-slate-500">
              Uncollected money from orders currently out with customers.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm bg-white hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs mt-2 text-slate-500">
              Advance payments received for upcoming bookings.
            </p>
          </CardContent>
        </Card>
        ...
        <Link href="/dashboard/orders?status=returned&status=completed&payment_status=partial&payment_status=pending&payment_status=due">
          <Card
            className={`border-0 shadow-sm hover:shadow-md transition-shadow cursor-pointer ${metrics.revenueByStatus.pendingAmount > 0 ? 'bg-rose-50/50' : 'bg-white'}`}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Clock
                className={`h-4 w-4 ${metrics.revenueByStatus.pendingAmount > 0 ? 'text-rose-400' : 'text-slate-400'}`}
              />
            </CardHeader>
            <CardContent>
              <div
                className={`text-3xl font-bold tracking-tight ${metrics.revenueByStatus.pendingAmount > 0 ? 'text-rose-700' : 'text-slate-900'}`}
              >
                {formatCurrency(metrics.revenueByStatus.pendingAmount)}
              </div>
              <p className="text-xs mt-2 text-slate-500">
                Unpaid balance for items already returned →
              </p>
            </CardContent>
          </Card>
        </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Cancellations */}
        <Card
          className={`border-0 shadow-sm ${metrics.cancellationStats.currentCount > 0 ? 'bg-rose-50/50' : 'bg-white'}`}
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Cancellations</CardTitle>
            <XCircle
              className={`h-4 w-4 ${metrics.cancellationStats.currentCount > 0 ? 'text-rose-500' : 'text-slate-400'}`}
            />
          </CardHeader>
          <CardContent>
            <div
              className={`text-3xl font-bold tracking-tight ${metrics.cancellationStats.currentCount > 0 ? 'text-rose-700' : 'text-slate-900'}`}
            >
        ...
        {/* Overdue Returns */}
        <Card
          className={`border-0 shadow-sm ${metrics.actionRequired.overdueCount > 0 ? 'bg-rose-50/50' : 'bg-white'}`}
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Overdue Returns</CardTitle>
            <AlertCircle
              className={`h-4 w-4 ${metrics.actionRequired.overdueCount > 0 ? 'text-rose-500' : 'text-slate-400'}`}
            />
          </CardHeader>
          <CardContent>
            <div
              className={`text-3xl font-bold tracking-tight ${metrics.actionRequired.overdueCount > 0 ? 'text-rose-700' : 'text-slate-900'}`}
            >
              {metrics.actionRequired.overdueCount} Orders
            </div>
            <p
              className={`text-xs mt-2 ${metrics.actionRequired.overdueCount > 0 ? 'text-rose-600 font-medium' : 'text-slate-500'}`}
            >
          ...
          <CardContent className="h-[250px] flex items-end gap-2 pt-4">
            {metrics.bookingVelocity.map((day, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex-1 bg-slate-100 hover:bg-slate-200 rounded-t-sm relative group transition-colors"
                  style={{ height: '100%' }}
                >
                  <div
          ...
          <CardContent className="pt-6">
            {metrics.categoryRevenue.length > 0 ? (
              ...
              <div className="py-8 text-center text-slate-500 text-sm">
                No category revenue data for this period.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm bg-white overflow-hidden flex flex-col">
          <CardHeader className="border-b border-slate-50 bg-slate-50/50 flex flex-row items-center justify-between space-y-0 py-3 px-6">
            <div>
              ...
              <CardDescription className="text-[10px]">
                Top {roiLimit} revenue contributors
              </CardDescription>
            </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {metrics.topPerformers.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-3.5 text-right text-emerald-600 font-semibold">
            ...
            {metrics.bottlenecks.length > 0 ? (
              <div className="space-y-3">
                {metrics.bottlenecks.map((item) => (
                  ...
                  >
                    <div
                      className={`p-2 rounded-lg shrink-0 ${item.severity === 'high' ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'}`}
                    >
                      {item.type === 'cleaning' ? (
